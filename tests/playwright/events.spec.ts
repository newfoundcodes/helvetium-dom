...
test.describe('native event system', () => {
      ...
      H.createRoot(document.querySelector('#app')!).render(
        H.h('input', {
          onInput: (e: Event & { currentTarget: HTMLInputElement }) =>
            (window.__value = e.currentTarget.value),
        }),
      );
    });
  ...
  test('handles keyboard events', async ({ page }) => {
    ...
    await page.evaluate(async () => {
      ...
      H.createRoot(document.querySelector('#app')!).render(
        H.h('input', { onKeyDown: (e: KeyboardEvent) => (window.__key = e.key) }),
      );
    });
  ...
  test('passive listeners cannot cancel a cancelable event', async ({ page }) => {
    ...
    const prevented = await page.evaluate(async () => {
      const H = (await import('/dist/index.js' as string)) as HelvetiumDOM;
      const a = document.querySelector('#app')!;
      H.createRoot(a).render(
        H.h('div', { onWheelPassive: (e: WheelEvent) => e.preventDefault() }, 'x'),
      );
